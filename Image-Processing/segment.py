...
def auto_segment_all_datasets():
    ...
    #accuracy_csv.write("Predicted EDV, Predicted ESV, Ejection Fraction\n") #Actual EDV,Actual ESV,
...
def segment_dataset(dataset):
   ...
   # log("Calculating ef...", 1)
    ...
    esv = min(area_totals)
    ef = (edv - esv) / edv
    if (ef > 0.4 and ef < 0.6):
	ef = ef * 0.1335 + 0.5336
    print "edv:",edv,"esv:",esv,"ef:",ef
    #log("Done, edv is %f" % edv, 1)
    #log("Done, esv is %f" % esv, 1)
